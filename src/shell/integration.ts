...
export class ShellIntegration {
  ...
  printInstructions(shell?: ShellType): void {
    ...
    if (targetShell === 'unknown') {
      ...
      console.log(chalk.dim('  You can still use clai directly - commands will be copied to clipboard.\n'));
      return;
    }
    ...
    console.log(chalk.green(snippet.split('\n').map(line => `  ${line}`).join('\n')));
    console.log(chalk.dim('  ─'.repeat(40)));

    console.log(chalk.bold('\n  Step 2: Add it to your shell config file:'));
    console.log(chalk.dim(`  Open ${configFile} in your editor and paste the snippet at the end.\n`));
